...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="Trello Burn-down Generator",
    ...
    parser.add_argument(
        "--board-url", "-burl", type=str, help="URL of the Trello board [default: None]"
    )
    parser.add_argument(
        "--member",
        "-m",
    ...
    parser.add_argument(
        ...
        "-ed",
        type=valid_date,
        help="end date of the sprint in DD.MM.YY format [default: today i. e. %s)"
        % today_str,
        default=today_str,
    )
    ...
    logger.info(
        "Creating burn-down chart for sprint %s - %s"
        % (
            config.start_date.strftime(DATE_FORMAT),
            config.end_date.strftime(DATE_FORMAT),
        )
    )
    ...
    _, _, board_cards = parse_board(logger, config, board_json)

    df = create_df(logger, config, board_cards)
